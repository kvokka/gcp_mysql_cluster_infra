...
source "googlecompute" "mysql-image" {
  ...
  image_family                = "ubuntu-2004-mysql"
  zone                        = var.zone
...
build {
  ...
  provisioner "shell" {
    ...
    inline = [
      ...
      "DEBIAN_FRONTEND='noninteractive' apt install -y percona-server-server-5.7 percona-xtrabackup-24 percona-toolkit qpress",
  ...
  # MySQL 5.7 made server-id field mandatory and require some value to start
  provisioner "file" {
    source = "files/mysql/12345_server_id.cnf"
    destination = "//etc/mysql/my.conf.d/12345_server_id.cnf"
  }
